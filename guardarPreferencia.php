...
$usuario = $_SESSION['usuario']; // Suponemos que ya tienes la sesión iniciada

if (session_status() === PHP_SESSION_ACTIVE) {
    #echo "La sesión está iniciada ".$usuario;
  } else {
    echo "La sesión no está iniciada";
  }

$query = "INSERT INTO preferencias (correo, tipoDestino) VALUES ('$usuario', '$tipoDestino')";
$resultado = $conexion->query($query);

if ($resultado) {
    #header("Location: index.php"); // Redireccionar al sitio principal
    echo "Se registró la preferencia";
} else {
    echo "Error al guardar la preferencia";
}
?>
<!DOCTYPE html>
<html lang="en">
        ...
        <div class="container pb-5">
            <div class="row">
                <div class="col-lg-5 mt-5">
                    <div class="card mb-3">
                    <?php
                        $conexion = new mysqli("localhost", "root", "", "destinosturisticos");
    
                        if($conexion){        
                            $query = "SELECT nombreDestino, ubicacion, precio, imagen FROM destinos
                                    WHERE idDestino = '$idDestino';";
                            $resultado = $conexion->query($query);

                            while($row=$resultado->fetch_assoc()){
                                $nombre = $row['nombreDestino'];
                                $ubicacion=$row['ubicacion'];
                                $precio=$row['precio'];
                                $imagen=$row['imagen'];

                            ?><img class="img-fluid" src="data:image/jpg;base64,<?php echo base64_encode($row['imagen']) ?>" alt="" width="500"><?php
                            #echo "hola".$nombre.$ubicacion.$precio;
                        }
                    }else{
                        echo "No conectó";
                    }  
                ...
                <div class="col-lg-7 mt-5">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <p class="h3 py-2"><?php echo $ubicacion;?></p>
                            <p class="py-2">
                            ...
                            <h6>Descripción:</h6>
                            <p>Hermoso lugar, tranquilo para disfrutar en familia y amigos, ademas, ofrece un ambiente armónico donde se puede relajar tranquilamente. También en horarios nocturnos se presenta un ambiente mas festivo y alegre, alrededor en este hermoso sitio turístico. Ven a disfrutar y principalmente a recargar energías</p>
                            ...
                            <button type="submit" class="btn btn-success btn-lg" name="submit" value="addtocard">Reservar</button>
                        </div>
                    </div>
                </div>
            </div>
